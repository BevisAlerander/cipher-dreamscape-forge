...
export class FhevmDecryptionSignature {
  ...
  static checkIs(s: unknown): s is FhevmDecryptionSignatureType {
    ...
    for (const addr of (s as any).contractAddresses as unknown[]) {
    ...
    if (!("eip712" in s && typeof (s as any).eip712 === "object")) {
      return false;
    }

    const eip = (s as any).eip712;
    ...
    if (!("types" in eip && typeof eip.types === "object")) {
      return false;
    }
  ...
  static async loadOrSign(
    ...
    try {
      ...
      console.log(
        `signature saved! contracts=${sig.contractAddresses.length}`,
      );
    } catch {
      console.error(
